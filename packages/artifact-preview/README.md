...
This package lives in the [dsh-plugins](../..) collection, which uses pnpm
workspaces. From the repository root:

```sh
pnpm install
pnpm --filter @hustshawn/dsh-artifact-preview run typecheck
pnpm --filter @hustshawn/dsh-artifact-preview run test
pnpm --filter @hustshawn/dsh-artifact-preview run build
```

Build and test wiring comes from `@hustshawn/dsh-plugin-shared`, which owns the
ModuleLoader wrapper and the list of shell-provided externals, so this package's
`tsdown.config.ts` only names its entries.
